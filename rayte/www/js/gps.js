var GPS = {
    ...
    llamarOperadora: function () {
        window.open('tel:0180000rayte', '_system');
    },
    /*
    ...
    codeLatLng: function (latlng, pin, centrar) {
        GPS.geocoder.geocode({'latLng': latlng}, function (results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                ...
                //$('#popover-preview-origen').val(direccion);
    ...
    validarCodeAddressOrigen: function (results, status) {
        switch (status) {
            case google.maps.GeocoderStatus.OK:
                GPS.pin.usuario.setPosition(results[0].geometry.location);
                var dir = results[0].address_components;
                var direccion = dir[1].short_name + ' ' + dir[0].short_name + ', ' + dir[2].short_name;
                GPS.pin.usuario.address = direccion;
                GPS.calcularRuta();
                break;
            default:
                GPS.agregaMensaje("Ocurrio un error", true);
    ...
    cambiarOrigenDestino: function (tipo) {

        switch (tipo) {
            case 'origen':
                //GPS.swalPreloader('Buscando...');
                var address = document.getElementById("popover-preview-origen").value;
                GPS.geocoder.geocode({'address': address, bounds: GPS.pin.usuario.cityBounds, region: 'MX'}, GPS.validarCodeAddressOrigen);
                break;
            case 'destino':
                //GPS.swalPreloader('Buscando...');
                var address = document.getElementById("popover-preview-destino").value;
                GPS.geocoder.geocode({'address': address, bounds: GPS.pin.usuario.cityBounds, region: 'MX'}, GPS.validarCodeAddress);
                break;
        }
    },
    mostrarPreview: function () {
        ...
        if (GPS.taxiSeleccionado()) {
            $$('#popover-preview-origen').val(GPS.pin.usuario.address);
            $$('#popover-preview-destino').val(GPS.pin.destino.address);
            GPS.calcularRuta();

            $$('#popover-preview-arrow').off('touchend').on('touchend', function () {
                GPS.centrarMapa();
                GPS.calcularRuta();
            });

            $$('#popover-preview-cancel').off('touchend').on('touchend', function (e) {
                console.log(window.event);
                GPS.muestraRutaMapa();
            });
            $$('#popover-preview-button').off('touchend').on('touchend', function (e) {
                if ($$('#popover-preview-origen').val() && $$('#popover-preview-destino').val()) {
                    ...
                    swal('Error', 'El origen y el destino son requeridos', 'warning');
                }
            });
    ...
    taxiSeleccionado: function () {
        GPS.tipoTaxi = GPS.menuSwiper.isBeginning ? 1 : GPS.menuSwiper.isEnd ? 2 : 0;
        return GPS.menuSwiper.isBeginning || GPS.menuSwiper.isEnd;
    },
    /*
    ...
    iniciaMapa: function () {
        ...
        $$('#popover-preview-origen').blur(function (e) {
            if (!$$(this).val()) {
                $('#popover-preview-origen').val(GPS.pin.usuario.address);
            } else {
                GPS.cambiarOrigenDestino('origen');
            }
        });

        $$('#popover-preview-destino').blur(function (e) {
            if (!$$(this).val()) {
                $('#popover-preview-destino').val(GPS.pin.destino.address);
            } else {
                GPS.cambiarOrigenDestino('destino');
            }
        });


        $$('#popover-preview-origen,#popover-preview-destino').on('keyup', function (e) {
        ...
        $$(document).on('touchstart', '#popover-confirm-yes', function (e) {
            e.preventDefault();
            GPS.muestraRutaMapa('taxi');
        ...
        $$(document).on('touchstart', '#popover-rate-button', function () {
            $$(this).addClass('active');
            //GPS.muestraRutaMapa();
            // rT = false;
        });

        $$(document).on('touchend', '#popover-rate-button', function (e) {
            $$(this).removeClass('active');
            GPS.muestraRutaMapa();
        });

        $$(document).on('touchcancel', '#popover-rate-button', function (e) {
            $$(this).removeClass('active');
        });
